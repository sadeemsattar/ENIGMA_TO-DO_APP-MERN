...
CREATE TABLE PRUSER(
    ...
    -- (is ko daal dena rest api mai bhi) 
);
select * from pruser;
drop table pruser;
...
CREATE TABLE MEMBERBOARD(
    ...
    MEMAIL Varchar(255),
    BID INT,
   --  foreign key(MEMAIL) references PRUSER(EMAIL),
...
CREATE TABLE CARD(
    ...
    LISTTYPE INT,
    foreign key(UID) REFERENCES PRUSER(UID),
    FOREIGN KEY(LISTTYPE) REFERENCES LISTTYPE(LTID)
); 

CREATE TABLE UTIMESTAMP(
	TID INT auto_increment PRIMARY KEY,
	BBID INT,
    TIMEDATE DATE NOT NULL,
    UPDATEDESCRIPTION VARCHAR(255) NOT NULL,
    foreign key(BBID) references MEMBERBOARD(MBID)
);
